#!/usr/bin/env python3
"""
TrotBot GUI Calibration Tool
Graphical interface for servo calibration
"""
...
class CalibrationTool:
    ...
    def __init__(self,title, width, height):
        ...
        self.MainWindow.title('TrotBot')   #TrotBot Calibration Tool
        ...
        #init robot image
        try:
            self.photo = tk.PhotoImage(file=Path(__file__).with_name('TrotBot.Calibration.png'))
        except tk.TclError:
            # Image not found, create a simple placeholder
            self.photo = None
        if self.photo:
            self.MainImg = Label(self.MainWindow,image = self.photo)
            self.MainImg.place(x=230,y=60)
        else:
            # Create a text label instead of image
            self.MainImg = Label(self.MainWindow, text="TrotBot\nCalibration Tool",
                               font=("Arial", 16, "bold"), fg="blue")
            self.MainImg.place(x=280,y=120)
    ...
    def readCalibrationFile(self):
        ...
        try:
            data = servo_calibration.read_calibration()
    ...
    def writeCalibrationFile(self):
        ...
        servo_calibration.write_calibration(data)
...
def main():
    ...
    ##############################################    
    MainWindow = CalibrationTool('TrotBot Calibration Tool',800,500)

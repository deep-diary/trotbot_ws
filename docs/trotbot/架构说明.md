...
- `trotbot_can_bridge/power_sequence_node`
  - 订阅 `/joy`、`/power_sequence/command`，处理上电/下电/趴下组合键与话题指令。
  - 发布 `/power_sequence/gate_open` 控制 `motor_protocol_node` 启动门禁。
  - 在 `SoftStand/SoftProne` 阶段发布 `body_pose`（`stand_z`↔`prone_z` 参数插值）；**`ProneHold`** 为趴下保持使能态。

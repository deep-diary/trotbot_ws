...
- `trotbot_status_led/status_led_node`（**可选**，`trotbot/launch/trotbot_basic.launch.py` 中 **`use_status_led:=true`** 时随 **`trotbot_status_led/launch/status_led.launch.py`** 启动，默认不启用以免无硬件开发机失败）
  - 订阅 `/power_sequence/state`、`/power_sequence/gate_open`（后者 **QoS** 与 `power_sequence` 发布端一致：`transient_local` + `reliable`），按 **`trotbot_status_led/config/status_led_map.yaml`** 将状态映射为 **16×WS2812** RGB 帧；硬件输出由参数 **`ws2812_backend`** 选择：**`spidev`（SPI MOSI，默认）**、**`rockchip_mmap`（GPIO 寄存器 + `/dev/mem`）** 或 **`gpiod`**。设备不可用时**降级**（仅 WARN，不阻塞运动栈；见包 **README**）。
